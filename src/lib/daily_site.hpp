...
class Daily_site {
  ...
  public:
    Daily_site() = default;
    Daily_site(int time, int site_idx, int total, int bandwidth) : time_(time),
        site_idx_(site_idx), total_(total), total_bandwidth_(bandwidth),
        remain_bandwidth(bandwidth){}

    int SetTotal(int total) {total_ = total;}
