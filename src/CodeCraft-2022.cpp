...
void SystemManager::PresetMaxSites() {
    ...
    sort(max_site_indexes.begin(), max_site_indexes.end(), //¼ÓÉÏ=ÓÐ±ä»¯
         [&sites_copy](size_t l, size_t r) {
             ...
             //                return sites_copy[l].GetRefTimes() >
             //                sites_copy[r].GetRefTimes();
             //            }
             return sites_copy[l].GetTotalBandwidth() >
                    sites_copy[r].GetTotalBandwidth();
             //            if(sites_copy[l].GetRefTimes() ==
             //            sites_copy[r].GetRefTimes()) {
             //                return sites_copy[l].GetTotalBandwidth() >
             //                sites_copy[r].GetTotalBandwidth();
             //            }
    ...
    for (size_t site_idx : max_site_indexes) {
        ...
        for (size_t day = 0; day < client_demands_cpy.size(); day++) {
            ...
            for (auto &s : streams) {
                if (s.stream_size / clients_[s.cli_idx].GetSiteCount() >
                    site.GetRemainBandwidth()) {
                    continue;
                }
                if (s.stream_size == 0) {
                    continue;
                ...
                site.DecreaseBandwidth(s.stream_size /
                                       clients_[s.cli_idx].GetSiteCount());
                /*day_demand[cli_idx] *
                 * ((sites_[site_idx].GetTotalBandwidth()*1.0)/
        ...
        for (int j = 0; j < (int)(demands_.size() * 0.05); j++) {
            ...
            //                    its.push_back(it);
            //                }
            //                sort(its.begin(), its.end(), [cli_idx](ittype l,
            //                ittype r) {
            //                    return l->second[cli_idx] >
            //                    r->second[cli_idx];
            //                });
            //                for (auto it : its) {
            //                    //  for (auto it = need.begin(); it !=
            //                    need.end(); it++) { if (it->second[cli_idx] >
            //                    site.GetRemainBandwidth()) {
            //                        continue;
            //                    }
            ...
            //                    client_demands_cpy[day][cli_idx] -=
            //                    it->second[cli_idx]; it->second[cli_idx] = 0;
            //                }
            //            }
...
void SystemManager::GreedyAllocate(Demand &d, int day) {
    ...
    for (int max_site_idx : daily_full_site_indexes_[day]) {
        ...
        //      });
        //      for (auto it : its) {
        //          /1* for (auto it = need.begin(); it != need.end(); it++) {
        //          *1/ if (it->second[cli_idx] > site.GetRemainBandwidth()) {
        //              continue;
        //          }
        //          if (it->second[cli_idx] == 0) {
        //              continue;
